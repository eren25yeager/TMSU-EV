document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- PDF Generation (Screenshot Method - CORRECTED) ---
    ...
    // --- Display Results Function (Full Version) ---
    function displayResults(data) {
        ...
        const createListHTML = (keys) => {
            return keys.map(key => {
                const cleanKey = key.replace(/ of the task/g, '');
                const value = data[key] || 0;
                return `<li><span>${cleanKey}</span> <span class="value">${value}</span></li>`;
            }).join('');
        };
        ...
        const percentageHTML = `
            <div class="progress-ring">
                <svg width="100" height="100">
                    <circle class="progress-ring__circle" stroke="${getChartColors().borderColor}" stroke-width="6" fill="transparent" r="${radius}" cx="50" cy="50" style="stroke-dasharray: ${circumference} ${circumference}; stroke-dashoffset: ${offset};"/>
                </svg>
                <span class="progress-ring__text">${percentage}%</span>
            </div>`;

        screens.results.innerHTML = `
            <div class="result-card" style="grid-column: 1 / -1;"><h2>${data.Name || 'Unknown Member'}</h2><p>${data.Committee || 'N/A'}</p></div>
            <div class="results-grid">
                ...
                <div class="details-container">
                    <details class="collapsible-list" open><summary>Performance Metrics</summary><ul>${performanceListHTML}</ul></details>
                    <details class="collapsible-list" open><summary>Soft & Technical Skills</summary><ul>${softSkillsListHTML}</ul></details>
                </div>
        ...
        document.getElementById('download-pdf-button').addEventListener('click', () => {
            generateAndDownloadPDF(data.Name || 'Member');
        });
        ...
        document.getElementById('new-search-button').addEventListener('click', () => {
            ...
            gsap.to("#results-screen > *", {
                duration: 0.5, y: -40, opacity: 0, stagger: 0.05, ease: "power2.in",
                onComplete: () => {
                    switchScreen('login');
                    inputs.code.value = '';
                    gsap.from(".login-container > *", {
                        duration: 0.7, y: 30, opacity: 0, stagger: 0.1, ease: "power3.out"
                    });
                }
            });
        });
