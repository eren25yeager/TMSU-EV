document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================================================
    // --- PDF Generation (FINAL, MORE RELIABLE METHOD) ---
    // ==========================================================================
    async function generateAndDownloadPDF(memberName) {
        ...
        const socialLinks = resultsContainer.querySelector('.social-links'); // <-- Target the social links

        // --- Prepare UI for capture ---
        downloadBtn.textContent = 'GENERATING...';
        downloadBtn.disabled = true;
        if (actionButtons) actionButtons.style.display = 'none'; // Hide buttons for PDF
        if (socialLinks) socialLinks.style.display = 'none';   // <-- Hide social links before capture
        ...
        window.scrollTo(0, 0); // Scroll to top to ensure canvas captures from the beginning
        ...
        try {
            ...
            allOtherElements.forEach(el => el.style.display = ''); // Restore display
            resultsContainer.style.position = '';
            resultsContainer.style.top = '';
            resultsContainer.style.left = '';
            if (actionButtons) actionButtons.style.display = ''; // Show buttons again
            if (socialLinks) socialLinks.style.display = '';   // <-- Restore social links
    ...
    function displayResults(data) {
        ...
        screens.results.innerHTML = `
            ...
            <!-- Social Links Added Here for Results Screen -->
    ...
    if (inputs.code) {
        ...
        if (inputWrapper && caret) {
            ...
            inputs.code.addEventListener('focus', () => {
                inputWrapper.classList.add('focused');
                updateCaretPosition();
            });

            inputs.code.addEventListener('blur', () => {
