document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- إعدادات تأثير الماتريكس ---
    const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789@#$%^&*()*&^%+-/~{[|`]}';
    const fontSize = 16;
    const columns = Math.floor(canvas.width / fontSize);
    const drops = Array(columns).fill(1);
    const mainColor = '#ff073a'; // اللون الأحمر الرئيسي

    function drawMatrix() {
        // خلفية شبه شفافة لإعطاء تأثير الذيل للأحرف
        ctx.fillStyle = 'rgba(0, 0, 0, 0.05)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.fillStyle = mainColor; // لون الأحرف
        ...
        for (let i = 0; i < drops.length; i++) {
            ...
            // إعادة القطرة إلى الأعلى بشكل عشوائي لإنشاء تأثير غير منتظم
    ...
    window.addEventListener('resize', () => {
        ...
        // إعادة تهيئة القطرات لتناسب العرض الجديد
    ...
    // --- التحكم في تسلسل ظهور واختفاء المقدمة ---
    const loginScreen = document.getElementById('login-screen');
    loginScreen.style.opacity = '0';
    loginScreen.classList.remove('active');

    // 1. إظهار العنوان بعد 3 ثوانٍ
    setTimeout(() => {
        introTitle.style.opacity = '1';
    }, 3000);

    // 2. إخفاء المقدمة بالكامل بعد 7 ثوانٍ
    setTimeout(() => {
        artisticIntro.style.opacity = '0';
        
        // 3. بعد انتهاء تأثير التلاشي، قم بإيقاف الأنيميشن وإظهار شاشة تسجيل الدخول
        setTimeout(() => {
            cancelAnimationFrame(animationFrameId); // مهم: إيقاف الأنيميشن لتوفير الموارد
            artisticIntro.style.display = 'none';
            loginScreen.classList.add('active');
            loginScreen.style.opacity = '1';
            initializeApp(); // تهيئة باقي التطبيق (مثل particles.js)
            gsap.from(".login-container > *", { duration: 0.7, y: 30, opacity: 0, stagger: 0.1, ease: "power3.out" });
        }, 1500); // مدة تتوافق مع مدة الـ transition في CSS

    }, 7000); // المدة الإجمالية للمقدمة
}
    ...
    function initializeApp() {
        ...
        typeEffect(document.getElementById('animated-title'), "TMSU Evaluation");
